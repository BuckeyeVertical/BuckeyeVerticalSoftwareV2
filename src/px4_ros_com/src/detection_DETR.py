...
Key Behaviors:
  ...
  3. Data Structures:
       - `detection_labels_` holds the list of labels detected in the current cycle.
       - `detection_coordinates_` holds the corresponding bounding boxes for the current cycle.
...
class RTDETRDetector(Detector):
    ...
    def _create_tiles(self, image: PILImage.Image) -> List[Tuple[PILImage.Image, Tuple[int, int]]]:
        ...
        Adapted from user's provided code, with deduplication for positions.
        """
    ...
    def _process_image_with_tiles(self, image: PILImage.Image) -> sv.Detections:
        """
        Process an image by tiling it, running the model on each tile,
        combining results, and applying NMS. Adapted from user's provided code.
        """
    ...
    def detect(self, image: np.ndarray) -> List[DetectionResult]:
        """
        Detects objects using RT-DETR with tiling, filters for target classes.
        """
...
class ImageListenerNode(Node):
    ...
    def timer_callback(self):
        ...
        self.get_logger().info(f'Detection cycle complete. Found {len(results)} target objects.')
        ...
        if results:
            for idx, det_result in enumerate(results):
                ...
                self.get_logger().debug(f"Detected: {det_result.label} at ({x},{y},{w},{h})")
        
        final_label_str = ', '.join(self.detection_labels_) if self.detection_labels_ else 'None'
        self.get_logger().info(f'Objects detected in current cycle: {final_label_str}')
